	...
	messages.forEach(function(message) {
		if (message.payload) {
			...
			if (message.app == 'payment') {
				messagesOut += message.app.substr(0, 1).toUpperCase() + message.app.substr(1) + ' in ' + (asset == 'null' ? 'pizza' : asset);
			}
					...
					if (message.payload) {
						...
						message.payload.inputs.forEach(function(input) {
							if (input.type && input.type == 'issue') {
								...
								if (objName) {
									messagesOut += '<div><span class="numberFormat">' + assocCommissions[objName][key].sum + '</span> pizza of ' + objName + ' commissions on <a href="#' + assocCommissions[objName][key].address + '">' + assocCommissions[objName][key].address + '</a>' +
...
function generateTransactionsList(objTransactions, address) {
	...
	for (var k in objTransactions) {
			...
			if (objFrom.issue) {
				...
				if (commissionName) {
					...
					listTransactions += '<div class="transactionUnitListAddress">' +
						...
						' Sum: <span class="numberFormat">' + objFrom.sum + '</span> pizza</div>' +
						'</div>';
				}
			}
			else {
				...
				listTransactions += '<div class="transactionUnitListAddress"><div>' + addressOut + '</div>' +
					'<div>(<span class="numberFormat">' + objFrom.amount + '</span> ' + (transaction.asset == null ? 'pizza' : transaction.asset) + ')</div></div>';
			}
		});
		...
		for (var k in transaction.to) {
			...
			listTransactions += '<div class="transactionUnitListAddress"><div>' + addressOut + '</div>' +
				'<div>(<span class="numberFormat">' + _addressTo.amount + '</span> ' + (transaction.asset == null ? 'pizza' : transaction.asset) + ', ' +
...
socket.on('addressInfo', function(data) {
	if (data) {
		...
		for (var k in data.objBalance) {
			if (k === 'pizza') {
				balance += '<div><span class="numberFormat">' + data.objBalance[k] + '</span> pizza</div>';
			}
			else {
		...
		if(data.unspent) {
			data.unspent.forEach(function(row) {
				listUnspent += '<div><a href="#' + row.unit + '">' + row.unit + '</a> (<span class="numberFormat">' + row.amount + '</span> ' + (row.asset == null ? 'pizza' : row.asset) + ')</div>';
			});
		}
